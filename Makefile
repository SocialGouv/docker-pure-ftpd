...
run: kill
	sudo docker run -d --name ftpd_server -p 2121:2121 -p 30000-30009:30000-30009 -e "PUBLICHOST=localhost" -e "ADDED_FLAGS=-d -d" pure-ftp-demo
...
test-bob:
	...
	ls -alh" | ftp -n -v -p localhost 2121
...
test-bob-tls:
	...
	echo '~ removed file ~'\n\
	ls -alh" | sudo lftp -u bob,test -e "set ssl:ca-file '$$cert'" localhost 2121
	cat test-new-file.txt
